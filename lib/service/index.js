...
class Passport {
  ...
  async signon (opts = {}, ips) {
    ...
    let doc = null
    if (userInfo.unionid) {
      doc = await this.wechatUser.findOne({'$or': [{'unionid': userInfo.unionid}, {'weapp.openid': userInfo.openid}]})
    } else {
      doc = await this.wechatUser.findOne({'weapp.openid': userInfo.openid})
    }
    let data = {
      ext: {
        wechat
  ...
  async login (opts = {}, ips = []) {
    // 从微信获取用户信息
    let doc = await this.weapp.get(`/auth/${opts.code}`)
    if (doc.err) throw error.err(doc)
    if (doc.errcode) {
      throw error.err({
        err: doc.errcode,
